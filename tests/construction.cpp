...
{
  xyz::copy_on_write<xyz::indirect<int>> x;
  EXPECT_EQ(*x, 0);
...
{
  xyz::copy_on_write<xyz::indirect<std::string>> x;
  EXPECT_EQ(*x, "");
  EXPECT_FALSE(x.valueless_after_move());
}
...
TEST(Construction, DefaultConstructorThrows)
{
  ...
  EXPECT_THROW(xyz::copy_on_write<xyz::indirect<ThrowOnConstruct>>(), std::runtime_error);
}
...
{
  xyz::copy_on_write<xyz::indirect<int>> x(42);
  EXPECT_EQ(*x, 42);
}
...
{
  std::string s = "hello";
  xyz::copy_on_write<xyz::indirect<std::string>> x(s);
  EXPECT_EQ(*x, "hello");
}

TEST(Construction, SingleValueConstructorRvalue)
{
  xyz::copy_on_write<xyz::indirect<std::string>> x(std::string("world"));
  EXPECT_EQ(*x, "world");
}
...
{
  xyz::copy_on_write<xyz::indirect<std::string>> x(std::in_place, 3u, 'a');
  EXPECT_EQ(*x, "aaa");
}

TEST(Construction, InPlaceInitializerList)
{
  xyz::copy_on_write<xyz::indirect<std::vector<int>>> x(std::in_place, {1, 2, 3});
...
TEST(Construction, InPlaceInitializerListExtraArgs)
{
  ...
  xyz::copy_on_write<xyz::indirect<std::vector<int>>> x(std::in_place, std::initializer_list<int>{4, 5, 6},
                                         std::allocator<int>{});
...
{
  xyz::copy_on_write<xyz::indirect<int>> x(std::allocator_arg, std::allocator<int>{});
  EXPECT_EQ(*x, 0);
  EXPECT_FALSE(x.valueless_after_move());
}
...
{
  xyz::copy_on_write<xyz::indirect<int>> x(std::allocator_arg, std::allocator<int>{}, 99);
  EXPECT_EQ(*x, 99);
}

TEST(Construction, AllocArgInPlaceConstructor)
{
  xyz::copy_on_write<xyz::indirect<std::string>> x(std::allocator_arg, std::allocator<std::string>{},
                                    std::in_place, 4u, 'z');
  EXPECT_EQ(*x, "zzzz");
}
...
{
  xyz::copy_on_write<xyz::indirect<std::vector<int>>> x(std::allocator_arg, std::allocator<std::vector<int>>{},
                                         std::in_place, {7, 8, 9});
...
{
  xyz::copy_on_write<xyz::indirect<std::string>> a("copy me");
...
TEST(Construction, CopyConstructorFromValueless)
{
  ...
  xyz::copy_on_write<xyz::indirect<int>> sink(std::move(a)); // a is now valueless
  xyz::copy_on_write<xyz::indirect<int>> b(a);              // copy-construct from valueless source
...
{
  xyz::copy_on_write<xyz::indirect<int>> a(7);
...
TEST(Construction, AllocArgCopyConstructorSameAllocatorSharesModel)
{
  ...
  xyz::copy_on_write<xyz::indirect<int>> a(std::allocator_arg, alloc, 21);
  xyz::copy_on_write<xyz::indirect<int>> b(std::allocator_arg, alloc, a);
  EXPECT_TRUE(a.identical_to(b));
  EXPECT_EQ(*b, 21);
}
...
TEST(Construction, AllocArgCopyConstructorFromValueless)
{
  ...
  xyz::copy_on_write<xyz::indirect<int>> sink(std::move(a)); // a is now valueless
  std::allocator<int> alloc;
  xyz::copy_on_write<xyz::indirect<int>> b(std::allocator_arg, alloc, a); // copy from valueless
  EXPECT_TRUE(b.valueless_after_move());
}
...
{
  std::allocator<int> alloc;
  xyz::copy_on_write<xyz::indirect<int>> a(std::allocator_arg, alloc, 33);
  xyz::copy_on_write<xyz::indirect<int>> b(std::allocator_arg, alloc, std::move(a));
  EXPECT_TRUE(a.valueless_after_move());
  EXPECT_EQ(*b, 33);
}
...
TEST(Construction, AllocArgMoveConstructorFromValueless)
{
  ...
  xyz::copy_on_write<xyz::indirect<int>> sink(std::move(a)); // a is now valueless
  std::allocator<int> alloc;
  xyz::copy_on_write<xyz::indirect<int>> b(std::allocator_arg, alloc, std::move(a)); // move from valueless
  EXPECT_TRUE(b.valueless_after_move());
}
...
TEST(Construction, DeductionGuideFromValue)
{
  ...
  static_assert(std::is_same_v<decltype(x), xyz::copy_on_write<xyz::indirect<int>>>);
  EXPECT_EQ(*x, 42);
}
...
TEST(Construction, DeductionGuideFromAllocArgValue)
{
  ...
  static_assert(std::is_same_v<decltype(x), xyz::copy_on_write<xyz::indirect<double, std::allocator<double>>>>);
